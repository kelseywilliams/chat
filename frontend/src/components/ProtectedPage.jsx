...
const ProtectedPage = () => {
    const { user, authLost, socket, connect_error } = useSessionStore();

    const isVerifying = socket && !user && !connect_error && !authLost;
    const hasAuthFailed = !user || authLost || connect_error;

    useEffect(() => {
        if (!isVerifying && hasAuthFailed) {
            window.location.replace("/login");
        }
    }, [isVerifying, hasAuthFailed]);
