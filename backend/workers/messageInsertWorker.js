...
export default async function startMessageInsertWorker(io) {
    ...
    const worker = new Worker(
        ...
        async (job) => {
            ...
            if (!cookie || !ulid || !room || !username || !content) {
                throw new UnrecoverableError(`Invalid job payload: ${JSON.stringify(job.data)}`);
            }
    ...
    worker.on("completed", (job, result) => {
        ...
        logger.info(`Worker created new chat in room=${room} job=${job.id}`);
    });

    worker.on("failed", (job, err) => {
        logger.error(`Worker failed job=${job?.id} status=${err?.status} err=${err?.message}`);
    });
